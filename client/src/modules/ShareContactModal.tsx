...
export default function ShareContactModal() {
  ...
  useEffect(() => {
    ...
    setUserContact(selectedContact);
  }, [selectedContact, state])

  const contactsOutput = allContacts?.map((contact, index) => {
    const fullname = `${toUpperFirst(contact?.first_name)} ${toUpperFirst(contact?.last_name)}`;
    if (userContact?.email === contact?.email) return null;
    return (
  ...
  return (
    ...
    <div className="modal-content">
      ...
      <div className="modal-body">
        <h5 className="mb-3 px-3">Share "{selectedContact?.email}" with:</h5>
